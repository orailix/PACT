...
def load_config(config_path=None):
    ...
    # Merge with defaults
    if "visual_token_reduction" in config.keys():
        default_config["visual_token_reduction"] =config["visual_token_reduction"]
...
class LlavaMetaForCausalLM(ABC):
    ...
    def add_token_per_grid(self, image_feature):
        ...
        if self.config.add_faster_video:
            ...
            # (channels, 832, 14) -> (channels, 64, 13, 14)
            image_feature = image_feature.view(feature_dim, num_frames,resize_h, -1)
            #  (channels, 64, 13, 14) -> (64, 13, 14, channels)
            image_feature = image_feature.permute(1, 2, 3, 0).contiguous()
            # (64, 13, 14, channels) -> (64, 13*14, channels)
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            if  hasattr(self,"max_position") :
                ...
                # print(f"self.max_position is {self.max_position} input_ids.shape[1] {input_ids.shape} {past_key_values[0][0].shape} {past_key_values[0][0].dtype}")
                #torch.tensor([self.max_position]).unsqueeze(0).to(self.device)
                #torch.arange(self.max_position+1,self.max_position+1+input_ids.shape[1])
                return input_ids[:,-1:], torch.tensor([self.max_position]).unsqueeze(0).to(self.device), attention_mask, past_key_values, None, labels
            else :
        ...
        if type(images) is list or images.ndim == 5:
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if mm_newline_position == "grid":
                            ...
                            if 'unpad' in mm_patch_merge_type:
                                ...
                                mask_image= torch.ones_like(image_feature)[:,0:1]
                                to_concat= torch.zeros((1,1))
                                to_concat=to_concat.to(image_feature.device)
                                mask_image = torch.cat((mask_image, to_concat), dim=0)
                        ...
                        if "maxpool2x2" in mm_patch_merge_type:
                            ...
                            to_concat= torch.zeros((*mask_image.shape[:-1], 1))
                            to_concat=to_concat.to(image_feature.device)

                            mask_image = torch.cat((mask_image, to_concat), dim=-1)
                            mask_image = mask_image.flatten(1, 2).transpose(0, 1)
                        ...
                        if "nobase" in mm_patch_merge_type:
                            ...
                            mask_base=torch.ones_like(base_image_feature)[:,0:1]
             
                            
                            mask_image = torch.cat((mask_base, mask_image), dim=0)
                        new_image_features.append(image_feature)
                        ...
                        if "unpad" in mm_patch_merge_type:
                            ...
                            to_concat= torch.zeros((1,1)).to(mask_image.dtype)
                            mask_image = torch.cat((mask_image, to_concat), dim=0)
                        else : 
                            mask_image=torch.ones_like(image_feature)
            ...
            mask_=torch.ones_like(image_features)[:,:,0:1]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                to_add=torch.zeros_like(cur_input_embeds_no_im[i])[:,0:1]
                cur_mask_.append(to_add)
                ...
                if i < num_images:
                    ...
                    cur_mask_.append(mask_[cur_image_idx])
                    # except IndexError:
            ...
            cur_mask_ = [x.to(self.device) for x in cur_mask_]
         
            cur_mask_ = torch.cat(cur_mask_)
            ...
            new_mask_.append(cur_mask_)

        # Truncate sequences to max length as image embeddings can make the sequence longer
        tokenizer_model_max_length = getattr(self.config, "tokenizer_model_max_length", None)
        # rank_print("Finishing Inserting")
        
        # for el in new_input_embeds :
        #     if el.shape[0]>tokenizer_model_max_length :
        #         print("!! did crop input !!")
        new_input_embeds = [x[-tokenizer_model_max_length:] for x, modality in zip(new_input_embeds, modalities)]
        new_mask_  = [x[-tokenizer_model_max_length:] for x, modality in zip(new_mask_, modalities)]
