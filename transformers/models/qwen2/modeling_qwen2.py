...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        if past_key_value is not None:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        #changed line as positions_ids max can be greater then it length whe nreduction is applied
        cos, sin = self.rotary_emb(value_states, seq_len=position_ids.max()+1)
        ...
        if pact_config.visual_token_reduction and weights is not None: 
            #change mask for proportionl attention
                ...
                if pact_config.visual_token_reduction :
                    ...
                    if image_in_input and is_reduction_layer and position_ids[:,:,1].bool().any()  : 
                        ...
                        if pact_config.token_pruning :
                            ...
                            if pact_config.pruning_filter_wth_percentage and pact_config.avoid_numerical_instability_prune:
