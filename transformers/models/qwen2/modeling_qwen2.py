...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    """
    # print(f"shapes are {position_ids.shape} {cos.shape} {q.shape} {cos[position_ids].unsqueeze(unsqueeze_dim).shape}")
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        # Handle the causal mask and attention mask
        causal_mask = attention_mask
        if attention_mask is not None:
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]

        if causal_mask is None:
            causal_mask = torch.tril(torch.ones((q_len, kv_seq_len), device=hidden_states.device)).unsqueeze(0).unsqueeze(0)
        ...
        # Determine the causal nature based on the mask
        is_causal = True if causal_mask is None and q_len > 1 else False

        # Manual scaled dot-product attention
        scale_factor = 1 / math.sqrt(query_states.size(-1))
        attn_weights = torch.matmul(query_states, key_states.transpose(-2, -1)) * scale_factor
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        bsz, q_len, _ = hidden_states.size()
        if keys!=None :
            key_states=keys.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        else :
            key_states = self.k_proj(hidden_states)
    ...
    def forward(
        ...
        for index,decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                image_in_input=len(position_ids.shape)==3
                if image_in_input :
                    real_position_ids=position_ids[:,:,0]
                ...
                if pact_config.visual_token_reduction :
                    ...
                    if image_in_input and  is_reduction_layer and position_ids[:,:,1].bool().any()  : 
                        
                        if pact_config.get_performance_metrics :
                            torch.cuda.synchronize()
                            start_algo=time.time()
                        is_image=position_ids[:,:,1].bool().squeeze()
                        if  pact_config.need_kq :
                            ...
                            if is_image_indices.numel() > 0:
                                ...
                                # Handle the case when there are no True values in is_image
                                last_true_idx = -1  # or any appropriate fallback value
                        ...
                        if pact_config.token_pruning :
                            ...
                            if pact_config.pruning_filter_wth_percentage :
                                ...
                                # Compute Q1 and Q3 using torch.quantile
                            ...
                            if pact_config.avoid_numerical_instability_prune:
                                ...
                                if num_selected < num_to_keep :
                                    ...
                                    # Calculate how many more elements are needed
                                    num_additional = num_to_keep - num_selected
                                    # Add elements from the tie_mask until we reach exactly num_to_keep
                                    additional_indices = tie_mask.nonzero(as_tuple=True)[0][:num_additional]
                                    first_mask[additional_indices] = True
                                elif num_selected > num_to_keep:
                                    tie_mask = (scores == thresh_scores)
                                    # Calculate how many elements need to be removed
                                    num_excess = num_selected - num_to_keep
                                    # Get indices of elements equal to the threshold
                                    excess_indices = tie_mask.nonzero(as_tuple=True)[0][:num_excess]
                                    # Exclude these excess elements from the mask
                                    first_mask[excess_indices] = False
                        ...
                        if pact_config.use_DBDPC and index==pact_config.layer_for_reduction :
                            ...
                            if not pact_config.include_pruned_in_mean :
                                merged,second_mask,weights,position_ids_after_reduction=token_reduction(hidden_states[:,first_mask_global].squeeze(0),vector_to_use_in_distance_clustering[:,first_mask_global].squeeze(0),pact_config=pact_config,position_ids=real_position_ids[:,first_mask_global].squeeze(0),reduction=self.reduction,cutoff=pact_config.cutoff)
                            else :
                                if pact_config.do_not_consider_non_image_tokens_as_pruned :
                                    merged,second_mask,weights,position_ids_after_reduction=token_reduction(hidden_states[:,first_mask_global].squeeze(0),vector_to_use_in_distance_clustering[:,first_mask_global].squeeze(0),pact_config=pact_config,position_ids=real_position_ids[:,first_mask_global].squeeze(0),reduction=self.reduction,cutoff=pact_config.cutoff,pruned_hiddens=hidden_states[:,is_image][:,~first_mask].squeeze(0),pruned_keys=vector_to_use_in_distance_clustering[:,is_image][:,~first_mask].squeeze(0))
                                else :
                                    merged,second_mask,weights,position_ids_after_reduction=token_reduction(hidden_states[:,first_mask_global].squeeze(0),vector_to_use_in_distance_clustering[:,first_mask_global].squeeze(0),pact_config=pact_config,position_ids=real_position_ids[:,first_mask_global].squeeze(0),reduction=self.reduction,cutoff=pact_config.cutoff,pruned_hiddens=hidden_states[:,~first_mask_global].squeeze(0),pruned_keys=vector_to_use_in_distance_clustering[:,~first_mask_global].squeeze(0))
                            if pact_config.get_mean_position_id:
                                position_ids_after_reduction=position_ids_after_reduction.to(real_position_ids.dtype)
                                position_ids[:,:,0][:,first_mask_global]=position_ids_after_reduction
                            ...
                            if index>=1 :
                                sizes=torch.exp(self.weights[index-1].squeeze()[first_mask_global])
                            else :
                            ...
                            merged,second_mask,weights,_=token_reduction_dbscan(hidden_states[:,first_mask_global].squeeze(0),vector_to_use_in_distance_clustering[:,first_mask_global].squeeze(0), eps=pact_config.eps_dbscan,noise_as_clusters_dbscan=pact_config.noise_as_clusters_dbscan,reduction=self.reduction)
                            hidden_states[:,first_mask_global]=merged.unsqueeze(0)
                            second_mask=second_mask.squeeze()
                            if pact_config.synchro or pact_config.get_reduction_ratio :
                                self.reduction[1]+=second_mask.sum().item()
