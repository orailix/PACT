    ...
    def forward(
        ...
        for index,decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                #     current_k=current_k.squeeze()[is_image]
                #     print(f"current_k shape is {current_k.shape}")

                #     normalized_k = torch.nn.functional.normalize(current_k, dim=1)

                #     cosine_similarity = torch.mm(normalized_k, normalized_k.T)
        
                #     # Convert to cosine distance (1 - cosine similarity)
                #     cosine_distance = 1 - cosine_similarity

                #     max_distance_keys = torch.max(cosine_distance).item()

                #     self.key_dist_stats[index][1]+=1
                #     self.key_dist_stats[index][0]+=max_distance_keys

                #     for key,value in self.key_dist_stats.items() :
                #         try :
                #             print(f" for layer {key} max distance is{value[0]/value[1]}")
                #         except :
                #             pass
                ...
                if pact_config.visual_token_reduction :
                    ...
                    if image_in_input and  is_reduction_layer and position_ids[3:,:,:].bool().any()  : 
                        ...
                        if  pact_config.need_kq :
                            # if prune_using_all_before_image:
                        ...
                        if pact_config.use_DBDPC and index==pact_config.layer_for_reduction :
                            ...
                            if not pact_config.include_pruned_in_mean :
                                ...
                                merged,second_mask,weights,position_ids_after_reduction=custom_token_reduction(hidden_states[:,first_mask_global].squeeze(0), vector_to_use_in_distance_clustering[:,first_mask_global].squeeze(0), position_ids=real_position_ids_after_mask_image, cutoff=pact_config.cutoff,pruned_hiddens=hidden_states[:,is_image][:,~first_mask].squeeze(0),pruned_keys=vector_to_use_in_distance_clustering[:,is_image][:,~first_mask].squeeze(0))
